...
function plot_in_subplots(t, y, t_steady_sate, y_steady_state)
  figure;

  subplot(2, 1, 1);
  plot(t, y(:, 1), 'Color', '#2BB735');
  title('Prey population over time');
  xlabel('time, years');
  ylabel('prey population');
  grid on
  hold on
  plot(t_steady_sate, y_steady_state(:, 1), 'Color', '#FEAB12')

  subplot(2, 1, 2);
  plot(t, y(:, 2), 'Color', '#0135E7');
